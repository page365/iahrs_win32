...
bool CSerialCOM::Open ()
{
	...
	if(_comHandle==INVALID_HANDLE_VALUE){
		OnFileAccessError ("CSerialCOM::Open (): CreateFile (): ");
		goto CLOSE_RET;
	}
	...
	if (!GetCommState (_comHandle, &dcb)) {	//dcb의 기본값을 받는다.
		OnFileAccessError ("CSerialCOM::Open (): GetCommState():");
		goto CLOSE_RET;
	}
	...
	if (!SetCommState(_comHandle, &dcb)) {
		OnFileAccessError ("CSerialCOM::Open (): SetCommState (): ");
		goto CLOSE_RET;
	}
...
void CSerialCOM::Purge ()
{
	...
	if (ClearCommError (_comHandle, &comError, &comStat)) {
		...
		OnFileAccessError ("CSerialCOM::Purge (): ClearCommError(): ");
	}
...
bool CSerialCOM::SetTimeout (int readTimeout, int writeTimeout, int readIntervalTimeout)
{
	...
	OnFileAccessError ("CSerialCOM::SetTimeout (): SetCommTimeouts(): ");
	return false;
}
...
int CSerialCOM::CountRx()
{
	...
	OnFileAccessError("CSerialCOM::CountRx (): ClearCommError(): ");
	return -1;
}

int CSerialCOM::CountTx()
{
	if (_comHandle == INVALID_HANDLE_VALUE) return -1;
	...
	if (ClearCommError(_comHandle, &comError, &comStat)) {
		if (comError) {
			TRACE("ERROR: CSerialCOM::CountTx(): %s\n", CommErrorString(comError));
		}
		return comStat.cbOutQue;
	}
	OnFileAccessError("CSerialCOM::CountTx (): ClearCommError(): ");
	return -1;
}
